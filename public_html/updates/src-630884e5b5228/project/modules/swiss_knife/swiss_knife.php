        ...
        foreach($settings['functions'] as $func)
            switch($func){
                ...
                case 'editPreviousMessage':
                    ...
                    $text=$settings['edit_previous_message_text'];
                    if($s){
                        if($text)
                            methods()->edit_message_text_or_caption($text, false, $s[0][0]['channel_id']??0, $s[0][0]['message_id']??0);
                        else
                            methods()->delete_mess($s[0][0]['channel_id']??0, $s[0][0]['message_id']);
                    }
