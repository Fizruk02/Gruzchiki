...
class input_datetime {
    ...
    private function datetimePicker($par){
	    ...
	    $block_dt_days=($data['block_dt_days']??0) && singleQuery('SELECT * FROM `dt_days` WHERE date="'.date('Y-m-d', $date).'"')?1:0;
        
		$arr =	array_map(function($value) use($date, $dataId, $block, $fraction, $block_dt_days) {
		    if($block_dt_days) return [ 'text'=> '✖️', 'callback_data'=>  json_encode(['mtd'=> 'dt01',  'dt'=> false]) ];
		    ...
		    if(is_array($block) && array_search((int) $strtotime/3600, $block)!==false)
		        return [ 'text'=> '✖️', 'callback_data'=>  json_encode(['mtd'=> 'dt01',  'dt'=> false]) ];
...
/**
    ...
    private function fractionKeyboard( array $par=[] ){
		...
		$arr =	array_map(function($value) use($dt) {
		    $current = date('Y-m-d H', $dt).':'.$value.':00';
		    $b = strtotime($current)>time()-60 ? true : false;$b=true;
