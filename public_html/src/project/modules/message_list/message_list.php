...
class message_list
{
    ...
    public function start(array $par = [])
    {
        ...
        $data['payload'] = $par;
        $data['update'] = $data['inline_previous_update']&&isset($obj['message']['reply_markup']['inline_keyboard'])?true:false;
        ...
        /**
         * Если список пуст и надо перейти на сценарий
         */
        if(isset($settings['script_if_empty']) && $settings['script_if_empty']){
            if(!singleQuery($data['query'])) {
                go_to($settings['script_if_empty'], 0, $par);
                return;
            }
        }
        ...
        switch ($settings['type']) {
            ...
            case 'inline_list':
                ...
                if($settings['custom_pagination_components']) {
                    $data['pagination_elements']=[
                        $settings['pagination_previous'],
                        $settings['pagination_to_start'],
                        $settings['pagination_to_end'],
                        $settings['pagination_next'],
                        $settings['pagination_empty']
                    ];
                }
                ...
                if ($settings['inlinemultiselect']) {
                    ...
                    if(!isset($settings['inlinemultiselectSelectAll'])) $settings['inlinemultiselectSelectAll']='';
                    if(!isset($settings['inlinemultiselectDeSelectAll'])) $settings['inlinemultiselectDeSelectAll']='';
                    if(!isset($settings['inlinemultiselectEndSelect'])) $settings['inlinemultiselectEndSelect']='';

                    $data['inlinemultiselectLimit'] = (int) $settings['inlinemultiselectLimit'];
                    if ($data['inlinemultiselectLimit']) {
                        $kb = [
                            [
                                ['text' => '❎ '.($settings['inlinemultiselectDeSelectAll']?:'Все'),
                        ...
                        $kb = [
                            [
                                ['text' => '✅ '.($settings['inlinemultiselectSelectAll']?:'Все'),
                    ...
                    $underPaginationKeys = [[
                        ['text' => ($settings['inlinemultiselectEndSelect']?:'ЗАКОНЧИТЬ ВЫБОР'), 'callback_data' => json_encode(['mtd' => 'message_list_inline_ms_ending', 'dataId' => $dataId])]
                    ]];
                    $data['lowerKeys'] = $kb;
                    $data['underPaginationKeys'] = $underPaginationKeys;
