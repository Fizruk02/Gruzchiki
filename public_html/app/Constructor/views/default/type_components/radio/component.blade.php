    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        ...
        @if(@$form['dataenum']!='')
            ...
            @foreach($dataenum as $k=>$d)
				...
				$checked = ( ($value && in_array($val, $value)) || (BTBooster::isCreate() && ($k==0 && @$form['validation'])) ) ? "checked" : "";
                ?>
                <div class=" {{$disabled}}">
        ...
        if (@$form['datatable']):
            ...
            if (BTBooster::isColumnExists($tables[0], 'deleted_at')) {
            ...
            if (count($tables)) {
                for ($i = 1; $i <= count($tables) - 1; $i++) {
                    ...
                    $fk_field = BTBooster::getForeignKey($parent_table, $tab);
                    $pk = BTBooster::findPrimaryKey($tab) ?: 'id';
