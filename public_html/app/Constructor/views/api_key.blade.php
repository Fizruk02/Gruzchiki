...
@section('content')
    ...
    <ul class="nav nav-tabs">
        <li><a href="{{ BTBooster::mainpath() }}"><i class='fa fa-file'></i> API Documentation</a></li>
        <li class="active"><a href="{{ BTBooster::mainpath('screet-key') }}"><i class='fa fa-key'></i> API Secret Key</a></li>
        <li><a href="{{ BTBooster::mainpath('generator') }}"><i class='fa fa-cog'></i> API Generator</a></li>
    </ul>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($row->status == 'active')
                                <a class='btn btn-xs btn-default' href='{{ BTBooster::mainpath("status-apikey?id=$row->id&status=0") }}'>Non Active</a>
                            @else
                                <a class='btn btn-xs btn-default' href='{{ BTBooster::mainpath("status-apikey?id=$row->id&status=1") }}'>Active</a>
                            @endif
                ...
                <script>
                    ...
                    function generate_screet_key() {
                        $.get("<?php echo route('ApiCustomControllerGetGenerateScreetKey')?>", function (resp) {
                            ...
                            $('#table-apikey').append("<tr><td>" + lastno + "</td><td>" + resp.key + "</td><td>0</td><td><span class='label label-success'>Active</span></td><td>" +
                                "<a class='btn btn-xs btn-default' href='{{BTBooster::mainpath("status-apikey")}}?id=" + resp.id + "&status=0'>Non Active</a> <a class='btn btn-xs btn-danger' href='javascript:void(0)' onclick='deleteApi(" + resp.id + ")'>Delete</a> </td></tr>"
                            );
                            $('.no-screetkey').remove();
                    ...
                    function deleteApi(id) {
                        swal({
                            ...
                            $.get("{{BTBooster::mainpath('delete-api-key')}}?id=" + id, function (resp) {
                                if (resp.status == 1) {
