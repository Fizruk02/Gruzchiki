...
@if($button_action_style == 'button_text')
    ...
    @if(BTBooster::isRead() && $button_detail)
        <a class='btn btn-xs btn-primary btn-detail' title='{{cbLang("action_detail_data")}}'
           href='{{BTBooster::mainpath("detail/".$row->$pk)."?return_url=".urlencode(Request::fullUrl())}}'>{{cbLang("action_detail_data")}}</a>
    @endif

    @if(BTBooster::isUpdate() && $button_edit)
        <a class='btn btn-xs btn-success btn-edit' title='{{cbLang("action_edit_data")}}'
           href='{{BTBooster::mainpath("edit/".$row->$pk)."?return_url=".urlencode(Request::fullUrl())."&parent_id=".g("parent_id")."&parent_field=".$parent_field }}'>{{cbLang("action_edit_data")}}</a>
    @endif

    @if(BTBooster::isDelete() && $button_delete)
        <?php $url = BTBooster::mainpath("delete/".$row->$pk);?>
        <a class='btn btn-xs btn-warning btn-delete' title='{{cbLang("action_delete_data")}}' href='javascript:;'
           onclick='{{BTBooster::deleteConfirm($url)}}'>{{cbLang("action_delete_data")}}</a>
    @endif
@elseif($button_action_style == 'button_icon_text')
    ...
    @if(BTBooster::isRead() && $button_detail)
        <a class='btn btn-xs btn-primary btn-detail' title='{{cbLang("action_detail_data")}}'
           href='{{BTBooster::mainpath("detail/".$row->$pk)."?return_url=".urlencode(Request::fullUrl())}}'><i
                    class='fa fa-eye'></i> {{cbLang("action_detail_data")}}</a>
    @endif

    @if(BTBooster::isUpdate() && $button_edit)
        <a class='btn btn-xs btn-success btn-edit' title='{{cbLang("action_edit_data")}}'
           href='{{BTBooster::mainpath("edit/".$row->$pk)."?return_url=".urlencode(Request::fullUrl())."&parent_id=".g("parent_id")."&parent_field=".$parent_field }}'><i
    ...
    @if(BTBooster::isDelete() && $button_delete)
        <?php $url = BTBooster::mainpath("delete/".$row->$pk);?>
        <a class='btn btn-xs btn-warning btn-delete' title='{{cbLang("action_delete_data")}}' href='javascript:;'
           onclick='{{BTBooster::deleteConfirm($url)}}'><i class='fa fa-trash'></i> {{cbLang("action_delete_data")}}</a>
    @endif
        ...
        <ul class='dropdown-menu dropdown-menu-action' role='menu'>
            ...
            @if(BTBooster::isDelete() && $button_delete)
                ...
                <li><a class='btn-delete' title='{{cbLang("action_delete_data")}}' href='javascript:;'
                       onclick='{{BTBooster::deleteConfirm($url)}}'><i class='fa fa-trash'></i> {{cbLang("action_delete_data")}}</a></li>
            @endif
        </ul>
    </div>
...
@else
    ...
    @if(BTBooster::isRead() && $button_detail)
        <a class='btn btn-xs btn-primary btn-detail' title='{{cbLang("action_detail_data")}}'
           href='{{BTBooster::mainpath("detail/".$row->$pk)."?return_url=".urlencode(Request::fullUrl())}}'><i class='fa fa-eye'></i></a>
    @endif
    ...
    @if(BTBooster::isDelete() && $button_delete)
        <?php $url = BTBooster::mainpath("delete/".$row->$pk);?>
        <a class='btn btn-xs btn-warning btn-delete' title='{{cbLang("action_delete_data")}}' href='javascript:;'
           onclick='{{BTBooster::deleteConfirm($url)}}'><i class='fa fa-trash'></i></a>
    @endif
