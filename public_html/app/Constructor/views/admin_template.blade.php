...
<head>
    ...
    <meta name='generator' content='BTBooster {{ \App\Constructor\commands\CrudboosterVersionCommand::$version }}'/>
    <meta name='robots' content='noindex,nofollow'/>
    <link rel="shortcut icon"
          href="{{ BTBooster::getSetting('favicon')?asset(BTBooster::getSetting('favicon')):asset('vendor/crudbooster/assets/logo_crudbooster.png') }}">
...
<!-- Content Wrapper. Contains page content -->
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            @if($module)
                <h1>
                    <!--Now you can define $page_icon alongside $page_tite for custom forms to follow BTBooster theme style -->
                    <?php $page_icon = $page_icon ?? null; ?>
                    <i class='{!! ($page_icon)?:$module->icon !!}'></i> {!! ucwords((@$page_title)?:$module->name) !!} &nbsp;&nbsp;

                    <!--START BUTTON -->

                    @if(BTBooster::getCurrentMethod() == 'getIndex')
                        @if($button_show)
                            <a href="{{ BTBooster::mainpath().'?'.http_build_query(Request::all()) }}" id='btn_show_data' class="btn btn-sm btn-primary"
                               title="{{cbLang('action_show_data')}}">
                                <i class="fa fa-table"></i> {{cbLang('action_show_data')}}
                            </a>
                        @endif

                        @if($button_add && BTBooster::isCreate())
                            <a href="{{ BTBooster::mainpath('add').'?return_url='.urlencode(Request::fullUrl()).'&parent_id='.g('parent_id').'&parent_field='.$parent_field }}"
                    ...
                    @if($button_export && BTBooster::getCurrentMethod() == 'getIndex')
                    ...
                    @if($button_import && BTBooster::getCurrentMethod() == 'getIndex')
                        <a href="{{ BTBooster::mainpath('import-data') }}" id='btn_import_data' data-url-parameter='{{$build_query}}' title='Import Data'
                ...
                <ol class="breadcrumb">
                    <li><a href="{{BTBooster::adminPath()}}"><i class="fa fa-dashboard"></i> {{ cbLang('home') }}</a></li>
