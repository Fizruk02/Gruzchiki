...
<form id='form-table' method='post' action='{{BTBooster::mainpath("action-selected")}}'>
    ...
    <table id='table_dashboard' class="table table-hover table-striped table-bordered">
		...
		$style = (isset($col['style'])) ?$col['style']: "";
            ...
            @if($button_table_action)
                @if(BTBooster::isUpdate() || BTBooster::isDelete() || BTBooster::isRead())
        ...
        @foreach($html_contents['html'] as $i=>$hc)
            ...
            @else
                <tr>
                    ...
                    @foreach($hc as $j=>$h)
                        <?php $columns[$j]['style'] = $columns[$j]['style'] ?? ''; ?>
        ...
        <tr>
            ...
            <?php
            foreach ($columns as $col) {
                $col['visible'] = isset($col['visible']) ? $col['visible'] : true;
                if ($col['visible'] === FALSE) continue;
                $colname = $col['label'] ?? '';
                $width = (isset($col['width'])) ?$col['width']: "auto";
		        $style = (isset($col['style'])) ? $col['style']: "";
                if (isset($col['label'])) echo "<th width='$width' $style>$colname</th>";
            }
            ?>
            @if($button_table_action)
                @if(BTBooster::isUpdate() || BTBooster::isDelete() || BTBooster::isRead())
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form method='get' action=''>
                        <div class="modal-body">
                            ...
                            <div class='form-group'>
                                ...
                                <div class='row-filter-combo row'>
                                    ...
                                    <div class='col-sm-3'>
                                        <?php
                                        $col["field_with"] = $col["field_with"] ?? '';
                                        $col["type_data"] = $col["type_data"] ?? '';
                                        ?>
                                        <select name='filter_column[{{$col["field_with"]}}][type]' data-type='{{$col["type_data"]}}'
                                            ...
                                            @if(in_array($col['type_data'],['string','varchar','text','char']))
                                                <option {{ (BTBooster::getTypeFilter($col["field_with"]) == 'like')?"selected":"" }} value='like'>{{cbLang("filter_like")}}</option> @endif
                                            @if(in_array($col['type_data'],['string','varchar','text','char']))
                                                <option {{ (BTBooster::getTypeFilter($col["field_with"]) == 'not like')?"selected":"" }} value='not like'>{{cbLang("filter_not_like")}}</option>@endif

                                            <option typeallow='all'
                                                    {{ (BTBooster::getTypeFilter($col["field_with"]) == '=')?"selected":"" }} value='='>{{cbLang("filter_equal_to")}}</option>
                                            @if(in_array($col['type_data'],['int','integer','smallint','tinyint','mediumint','bigint','double','float','decimal','time']))
                                                <option {{ (BTBooster::getTypeFilter($col["field_with"]) == '>=')?"selected":"" }} value='>='>{{cbLang("filter_greater_than_or_equal")}}</option>@endif
                                            @if(in_array($col['type_data'],['int','integer','smallint','tinyint','mediumint','bigint','double','float','decimal','time']))
                                                <option {{ (BTBooster::getTypeFilter($col["field_with"]) == '<=')?"selected":"" }} value='<='>{{cbLang("filter_less_than_or_equal")}}</option>@endif
                                            @if(in_array($col['type_data'],['int','integer','smallint','tinyint','mediumint','bigint','double','float','decimal','time']))
                                                <option {{ (BTBooster::getTypeFilter($col["field_with"]) == '<')?"selected":"" }} value='<'>{{cbLang("filter_less_than")}}</option>@endif
                                            @if(in_array($col['type_data'],['int','integer','smallint','tinyint','mediumint','bigint','double','float','decimal','time']))
                                                <option {{ (BTBooster::getTypeFilter($col["field_with"]) == '>')?"selected":"" }} value='>'>{{cbLang("filter_greater_than")}}</option>@endif
                                            <option typeallow='all'
                                                    {{ (BTBooster::getTypeFilter($col["field_with"]) == '!=')?"selected":"" }} value='!='>{{cbLang("filter_not_equal_to")}}</option>
                                            <option typeallow='all'
                                                    {{ (BTBooster::getTypeFilter($col["field_with"]) == 'in')?"selected":"" }} value='in'>{{cbLang("filter_in")}}</option>
                                            <option typeallow='all'
                                                    {{ (BTBooster::getTypeFilter($col["field_with"]) == 'not in')?"selected":"" }} value='not in'>{{cbLang("filter_not_in")}}</option>
                                            @if(in_array($col['type_data'],['date','time','datetime','int','integer','smallint','tinyint','mediumint','bigint','double','float','decimal','timestamp']))
                                                <option {{ (BTBooster::getTypeFilter($col["field_with"]) == 'between')?"selected":"" }} value='between'>{{cbLang("filter_between")}}</option>@endif
                                            <option {{ (BTBooster::getTypeFilter($col["field_with"]) == 'empty')?"selected":"" }} value='empty'>{{cbLang("filter_empty_or_null")}}</option>
                                        </select>
                                    </div><!--END COL_SM_4-->
                                    ...
                                    <div class='col-sm-5'>
                                        <input type='text' class='filter-value form-control'
                                               style="{{ (BTBooster::getTypeFilter($col["field_with"]) == 'between')?"display:none":"display:block"}}"
                                               disabled name='filter_column[{{$col["field_with"]}}][value]'
                                               value='{{ (!is_array(BTBooster::getValueFilter($col["field_with"])))?BTBooster::getValueFilter($col["field_with"]):"" }}'>

                                        <div class='row between-group'
                                             style="{{ (BTBooster::getTypeFilter($col["field_with"]) == 'between')?"display:block":"display:none" }}">
                                            <div class='col-sm-6'>
                                                <div class='input-group {{ ($col["type_data"] == "time")?"bootstrap-timepicker":"" }}'>
                                                    <span class="input-group-addon">{{cbLang("filter_from")}}:</span>
                                                    <input
                                                            {{ (BTBooster::getTypeFilter($col["field_with"]) != 'between')?"disabled":"" }}
                                                            type='text'
                                                            ...
                                                            value='<?php
                                                                $value = BTBooster::getValueFilter($col["field_with"]);
                                                                echo (BTBooster::getTypeFilter($col["field_with"]) == 'between') ? $value[0] : "";
                                                            ?>'>
                                                </div>
                                            </div>
                                            <div class='col-sm-6'>
                                                <div class='input-group {{ ($col["type_data"] == "time")?"bootstrap-timepicker":"" }}'>
                                                    ...
                                                    <input
                                                            ...
                                                            value='<?php
                                                                $value = BTBooster::getValueFilter($col["field_with"]);
                                                                echo (BTBooster::getTypeFilter($col["field_with"]) == 'between') ? $value[1] : "";
                                                            ?>'>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class='col-sm-2'>
                                        <select class='form-control' name='filter_column[{{$col["field_with"]}}][sorting]'>
                                            ...
                                            <option {{ (BTBooster::getSortingFilter($col["field_with"]) == 'asc')?"selected":"" }} value='asc'>{{cbLang("filter_ascending")}}</option>
                                            <option {{ (BTBooster::getSortingFilter($col["field_with"]) == 'desc')?"selected":"" }} value='desc'>{{cbLang("filter_descending")}}</option>
                                        </select>
                                    </div><!--END_COL_SM_2-->
                        ...
                        {!! BTBooster::getUrlParameters(['filter_column','lasturl']) !!}
        ...
        <div class="modal fade" tabindex="-1" role="dialog" id='export-data'>
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method='post' target="_blank" action='{{ BTBooster::mainpath("export-data?t=".time()) }}'>
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        {!! BTBooster::getUrlParameters() !!}
                        <div class="modal-body">
                            <div class="form-group">
                            ...
                            <div class='form-group'>
                                <label>{{cbLang("export_dialog_columns")}}d</label><br/>
                                @foreach($columns as $col)
                                    <?php $col["name"] = $col["name"] ?? ''; ?>
                                    <?php $col["label"] = $col["label"] ?? ''; ?>
                            ...
                            <div id='advanced_export' style='display: none'>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class='form-control' name='page_size'>
                                        <?php $default_paper_size = $setting->default_paper_size ?? null; ?>
                                        <option <?=($default_paper_size == 'Letter') ? "selected" : ""?> value='Letter'>Letter</option>
                                        <option <?=($default_paper_size == 'Legal') ? "selected" : ""?> value='Legal'>Legal</option>
                                        <option <?=($default_paper_size == 'Ledger') ? "selected" : ""?> value='Ledger'>Ledger</option>
                                        <?php for($i = 0;$i <= 8;$i++):
                                        $select = ($default_paper_size == 'A'.$i) ? "selected" : "";
                                        ?>
                                        <option <?=$select?> value='A{{$i}}'>A{{$i}}</option>
                                        <?php endfor;?>

                                        <?php for($i = 0;$i <= 10;$i++):
                                        $select = ($default_paper_size == 'B'.$i) ? "selected" : "";
                                        ?>
