        ...
        <div class="input-group">
            <div class="relative">
                ...
                <div x-data="{ datetimeFields: '' }" x-init="datetime_{{$name}};">
                    <input type='text' format="dd.mm.YYYY" datepicker title="{{$form['label']}}" readonly id="datetime_{{$name}}"
                           {{$required}} {{$readonly}} {!!$placeholder!!} {{$disabled}}
                           class='form-control notfocus pl-10 input_date transition-all duration-300 ease-in-out px-2 py-1 rounded cursor-pointer focus:outline-none hover:bg-white hover:border-gray-500 border border-transparent border-gray-300 w-full' name="{{$name}}" id="{{$name}}"
                           value='{{$value}}' />
                </div>
            </div>
        </div>
