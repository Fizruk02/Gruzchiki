<li class='{{ @$stripe && @$loop->index %2 === 0 ? 'bg-gray-100' : '' }} px-4 py-2 border-b border-gray-200 sm:flex sm:items-center form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    ...
    @endif
    <div class="{{$col_width?:'col-sm-10'}} mt-1 text-sm leading-5 sm:mt-0 @if ($form['label'] != null) sm:w-9/12 @else w-full @endif">
        @if ($form['label'] == null && $required) <div class="text-danger absolute px-1" style="user-select: none;">*</div> @endif
        <textarea rows=6 name="{{$form['name']}}" id="{{$name}}"
                  {{$required}} {{$readonly}} {!!$placeholder!!} {{$disabled}} {{@$validation['max']?"maxlength=".@$validation['max']:""}}
                  class='form-control transition-all duration-300 ease-in-out px-2 py-1 rounded cursor-pointer focus:outline-none hover:bg-white hover:border-gray-500 border border-transparent border-gray-300 w-full'
                  >{{ $value}}</textarea>
