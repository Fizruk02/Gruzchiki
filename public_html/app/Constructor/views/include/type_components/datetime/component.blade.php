        ...
        <div class="input-group">
            <div class="flex justify-left">
                <div class="relative">
                    ...
                    <input type='text' format="dd.mm.YYYY" datepicker title="{{$form['label']}}" readonly id="datetime_{{$name}}" name="{{$name}}"
                ...
                <div class="timepicker relative xl:w-96" data-mdb-with-icon="false" id="input-toggle-timepicker">
                    <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                        <svg aria-hidden="true" class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" clip-rule="evenodd"></path></svg>
                    </div>
                    <input type="text" readonly id="time_{{$name}}" name="time_{{$name}}"
                           class="form-control notfocus pl-10 input_date transition-all duration-300 ease-in-out px-2 py-1 rounded cursor-pointer focus:outline-none hover:bg-white hover:border-gray-500 border border-transparent border-gray-300 w-full"
                           placeholder="Выберите время" data-mdb-toggle="input-toggle-timepicker"
                           value='{{$time}}'
                    />
                </div>
            </div>
