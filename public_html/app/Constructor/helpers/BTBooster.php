    ...
    public static function myPrivilege()
    {
        ...
        if ($roles) {
            foreach ($roles as $role) {
                if ($role->path == BTBooster::getModulePath()) {
                    return $role;
                }
            }
        }
...
//$to = 'http://personal.local/cabinets';
    ...
    public static function getCurrentModule()
    {
        ...
        if (Cache::has('moduls_'.$modulepath)) {
            ...
            $module = DB::table('cms_moduls')->where('path', $modulepath)->first();
    ...
    public static function getModulePath()
    {
        ...
        $mp = Request::segment(@$segment);
        if ($mp == 'admin') $mp = Request::segment(@$segment + 1);
        return $mp;
    }
    ...
    public static function mainpath($path = null)
    {
        ...
        $controllername = str_replace(["\App\Constructor\controllers\\", "App\Http\Controllers\\"], "", strtok(Route::currentRouteAction(), '@'));
        //$controllername = 'AdminCmsUsersController';
    ...
    public static function sendEmail($config = [])
    {
        ...
        $template = BTBooster::first('cms_email_templates', ['slug' => $template]);
        ...
        if ($config['send_at'] != null) {
            ...
            $a['email_from_email'] = $template->from_email ?: BTBooster::getSetting('email_sender');
            $a['email_from_name'] = $template->from_name ?: BTBooster::getSetting('appname');
        ...
        \Mail::send("crudbooster::emails.blank", ['content' => $html], function ($message) use ($to, $subject, $template, $attachments) {
            ...
            if ($template->from_email) {
                $from_name = ($template->from_name) ?: BTBooster::getSetting('appname');
...
//         }
//         $table = BTBooster::parseSqlTable($table);
...
//     }
    ...
    {
        $key = BTBooster::findPrimaryKey($table);
    ...
    {
        $parent_table = BTBooster::parseSqlTable($parent_table)['table'];
        $child_table = BTBooster::parseSqlTable($child_table)['table'];
    ...
    public static function urlFilterColumn($key, $type, $value = '', $singleSorting = true)
    {
        ...
        if (isset($params['filter_column']) && $params['filter_column'] && $singleSorting) {
    ...
    {
        if (BTBooster::getSetting('api_debug_mode')) {
            $a = [];
    ...
    public static function sendNotification($config = [])
    {
        ...
        $id_cms_users = ($id_cms_users) ?: [BTBooster::myId()];
    ...
    public static function sendFCM($regID = [], $data)
    {
        ...
        $apikey = BTBooster::getSetting('google_fcm_key');
    ...
    public static function generateAPI($controller_name, $table_name, $permalink, $method_type = 'post')
    {
		...
		class Api'.$controller_name.'Controller extends \App\Constructor\controllers\ApiController {
    ...
    public static function generateController($table, $name = null)
    {
        ...
        $coloms = BTBooster::getTableColumns($table);
        $name_col = BTBooster::getNameTable($coloms);
        $pk = BT::pk($table);
...
<?php namespace App\Http\Controllers;
	...
	class Admin'.$controllername.' extends \App\Constructor\controllers\BTController {
	...
	';
        ...
        foreach ($coloms_col as $c) {
            ...
            if (substr($field, 0, 3) == 'id_') {
                ...
                $joincols = BTBooster::getTableColumns($jointable);
                $joinname = BTBooster::getNameTable($joincols);
        ...
        foreach ($coloms as $c) {
            ...
            $typedata = BTBooster::getFieldType($table, $field);
