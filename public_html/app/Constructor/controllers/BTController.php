<?php namespace App\Constructor\controllers;

error_reporting(E_ALL ^ E_NOTICE);


use App\Constructor\helpers\BT;
use App\Constructor\export\DefaultExportXls;
...
class BTController extends Controller
{
    ...
    public $ul_class = null;

    public $custom_component = null;
    ...
    public function cbLoader()
    {
        ...
        $this->primary_key = \App\Constructor\helpers\BT::pk($this->table);
        ...
        $this->data['appname'] = BTBooster::getSetting('appname');
        ...
        $this->data['button_exts'] = $this->button_exts;
        ...
        $this->data['ul_class'] = $this->ul_class;
        ...
        if (BTBooster::getCurrentMethod() == 'getProfile') {
            Session::put('current_row_id', BTBooster::myId());
    ...
    public function getIndex()
    {
        ...
        $module = BTBooster::getCurrentModule();

        if (! BTBooster::isView() && $this->global_privilege == false) {
            BTBooster::insertLog(cbLang('log_try_view', ['module' => $module->name]));
            BTBooster::redirect(BTBooster::adminPath(), cbLang('denied_access'));
        }

        if (request('parent_table')) {
            $parentTablePK = BT::pk(g('parent_table'));
            ...
            if (request('foreign_key')) {
                ...
                $data['parent_field'] = BT::getTableForeignKey(g('parent_table'), $this->table);
            }
        ...
        $data['table_pk'] = BT::pk($this->table);
        ...
        $table_columns = BT::getTableColumns($this->table);
        $result = DB::table($this->table)->select(DB::raw($this->table.".".$this->primary_key));

        if (request('parent_id')) {
            $table_parent = $this->table;
            $table_parent = BTBooster::parseSqlTable($table_parent)['table'];
        ...
        foreach ($columns_table as $index => $coltab) {
            ...
            if ($join) {
                ...
                $join_table_columns = BTBooster::getTableColumns($join_table);
                ...
                $alias[] = $join_alias;
                $columns_table[$index]['type_data'] = BTBooster::getFieldType($join_table, $join_column);
                ...
                @$joinTable1PK = BT::pk($join_table1);
                ...
                if ($join_table1 && $join_column1) {
                    ...
                    $alias[] = $join_alias1;
                    $columns_table[$index]['type_data'] = BTBooster::getFieldType($join_table1, $join_column1);
                ...
                if(isset($field_array[1])) {
                    ...
                    $columns_table[$index]['type_data'] = BTBooster::getFieldType($table, $field);
        ...
        if ($this->sub_module) {
            foreach ($this->sub_module as $s) {
                $table_parent = BTBooster::parseSqlTable($this->table)['table'];
                $addaction[] = [
                    'label' => $s['label'],
                    'icon' => $s['button_icon'],
                    'url' => BTBooster::adminPath($s['path']).'?return_url='.urlencode(Request::fullUrl()).'&parent_table='.$table_parent.'&parent_columns='.$s['parent_columns'].'&parent_columns_alias='.$s['parent_columns_alias'].'&parent_id=['.(! isset($s['custom_parent_id']) ? "id" : $s['custom_parent_id']).']&foreign_key='.$s['foreign_key'].'&label='.urlencode($s['label']),
        ...
        $mainpath = BTBooster::mainpath();
        $this->data['mainpath'] = $this->data['mainpath'] ?? $mainpath;
    ...
    public function getExportData()
    {
        ...
        return redirect(BTBooster::mainpath());
    }
    ...
    public function getModalData()
    {
        ...
        $table = BTBooster::parseSqlTable($table)['table'];
        $tablePK = BT::pk($table);
    ...
    public function getFindData()
    {
        ...
        if ($q || $id || $table1) {
            ...
            if (BTBooster::isColumnExists($table1, 'deleted_at')) {
            ...
            if ($table2 && $column2) {
                $table2PK = BT::pk($table2);
                $rows->join($table2, $table2.'.'.$table2PK, '=', $table1.'.'.$column1);
                $columns = BTBooster::getTableColumns($table2);
            ...
            if ($table3 && $column3) {
                $table3PK = BT::pk($table3);
                $rows->join($table3, $table3.'.'.$table3PK, '=', $table2.'.'.$column2);
                $columns = BTBooster::getTableColumns($table3);
                foreach ($columns as $col) {
                ...
                if ($exp && count($exp)) {
                    foreach ($exp as &$validationItem) {
                        if (substr($validationItem, 0, 6) == 'unique') {
                            ...
                            if (BT::isColumnExists($uniqueTable, 'deleted_at')) {
                                $uniqueRebuild[] = BT::findPrimaryKey($uniqueTable);
...
//dd($message_all);
    ...
    public function input_assignment($id = null, $is_edit = false)
    {
        ...
        foreach ($this->data_inputan as $ro) {
            ...
            if ($name) {
                if ($inputdata != '') {
                    ...
                    if (BT::isColumnNULL($this->table, $name) && @$ro['type'] != 'upload') {
                        if (!$is_edit && @$ro['type'] != 'upload') continue;
                        if (BT::isColumnNULL($this->table, $name)) $this->arr[$name] = null;
                        else $this->arr[$name] = "";
                    } else {
                        $this->arr[$name] = "";
                    }
            ...
            if (@$ro['type'] == 'checkbox') {
                ...
                if (is_array($inputdata)) {
                    if (@$ro['datatable'] != '') {
                        ...
                        $table_checkbox_pk = BT::pk($table_checkbox);
            ...
            if (@$ro['type'] == 'upload') {
                ...
                $this->arr[$name] = BTBooster::uploadFile($name, @$ro['encrypt'] || @$ro['upload_encrypt'], @$ro['resize_width'], @$ro['resize_height'], BT::myId());
    ...
    {
        $this->cbLoader();
        if (! BTBooster::isCreate() && $this->global_privilege == false || $this->button_add == false) {
            BTBooster::insertLog(cbLang('log_try_add', ['module' => BTBooster::getCurrentModule()->name]));
            BTBooster::redirect(BTBooster::adminPath(), cbLang("denied_access"));
        }
    ...
    {
        $this->cbLoader();
        if (! BTBooster::isCreate() && $this->global_privilege == false) {
            BTBooster::insertLog(cbLang('log_try_add_save', [
        ...
        foreach ($this->data_inputan as $ro) {
            ...
            if (@$ro['type'] == 'checkbox') {
                if ($ro['relationship_table']) {
                    ...
                    $foreignKey = BTBooster::getForeignKey($this->table, $ro['relationship_table']);
                    DB::table($ro['relationship_table'])->where($foreignKey, $id)->delete();

                    if ($inputdata) {
                        $relationship_table_pk = BT::pk($ro['relationship_table']);
                        foreach ($inputdata as $input_id) {
                            DB::table($ro['relationship_table'])->insert([
//                                 $relationship_table_pk => BTBooster::newId($ro['relationship_table']),
                                $foreignKey => $id,
            ...
            if (@$ro['type'] == 'child') {
                ...
                $childtable = BTBooster::parseSqlTable($ro['table'])['table'];
        ...
        //insert log
        BTBooster::insertLog(cbLang("log_add", ['name' => $this->arr[$this->title_field], 'module' => BTBooster::getCurrentModule()->name]));

        if ($this->return_url) {
            if (request('submit') == cbLang('button_save_more')) {
                BTBooster::redirect(Request::server('HTTP_REFERER'), cbLang("alert_add_data_success"), 'success');
            } else {
                BTBooster::redirect($this->return_url, cbLang("alert_add_data_success"), 'success');
            }
        } else {
            if (request('submit') == cbLang('button_save_more')) {
                BTBooster::redirect(BTBooster::mainpath('add'), cbLang("alert_add_data_success"), 'success');
            } else {
                BTBooster::redirect(BTBooster::mainpath(), cbLang("alert_add_data_success"), 'success');
            }
        }
    }

    public function getEditModel($row)
    {
        $this->cbLoader();

        if (! BTBooster::isRead() && ($this->global_privilege == false) || ($this->button_edit == false)) {
            BTBooster::insertLog(cbLang("log_try_edit", [
        ...
        $return_url = $this->return_url;
        $id = $row->id;
        $custom_component = $this->custom_component;
        Session::put('current_row_id', $id);

        return view(
            config("crudbooster.ADMIN_PATH") ? 'crudbooster::default.form' : 'crudbooster::include.form',
            compact('id', 'row', 'page_menu', 'page_title', 'command', 'return_url', 'custom_component')
        );
    }
    ...
    {
        $row = DB::table($this->table)->where($this->primary_key, $id)->first();
        return $this->postEdit($row);
    }

    public function postEdit($row)
    {
        $this->cbLoader();
        $id = $row->id;

        if (! BTBooster::isUpdate() && $this->global_privilege == false) {
            BTBooster::insertLog(cbLang("log_try_add", ['name' => $row->{$this->title_field}, 'module' => BTBooster::getCurrentModule()->name]));
            BTBooster::redirect(BTBooster::adminPath(), cbLang('denied_access'));
        }

        $this->validation($id);
        $this->input_assignment($id, true);
...
//                                 $relationship_table_pk => BTBooster::newId($ro['relationship_table']),
            ...
            if (@$ro['type'] == 'child') {
                ...
                $lastId = BTBooster::newId($childtable);
                $childtablePK = BT::pk($childtable);
    ...
    public function getDelete($id)
    {
        ...
        if (! BTBooster::isDelete() && $this->global_privilege == false || $this->button_delete == false) {
            BTBooster::insertLog(cbLang("log_try_delete", [
                'name' => $row->{$this->title_field},
                'module' => BTBooster::getCurrentModule()->name,
            ]));
            BTBooster::redirect(BTBooster::adminPath(), cbLang('denied_access'));
        ...
        //insert log
        BTBooster::insertLog(cbLang("log_delete", ['name' => $row->{$this->title_field}, 'module' => BTBooster::getCurrentModule()->name]));

        $this->hook_before_delete($id);

        if (BTBooster::isColumnExists($this->table, 'deleted_at')) {
        ...
        $url = g('return_url') ?: BTBooster::referer();

        BTBooster::redirect($url, cbLang("alert_delete_data_success"), 'success');
    }
    ...
    public function getDetail($id)
    {
        ...
        if (! BTBooster::isRead() && $this->global_privilege == false || $this->button_detail == false) {
            BTBooster::insertLog(cbLang("log_try_view", [
                'name' => $row->{$this->title_field},
                'module' => BTBooster::getCurrentModule()->name,
            ]));
            BTBooster::redirect(BTBooster::adminPath(), cbLang('denied_access'));
    ...
    public function postDoImportChunk()
    {
        ...
        if (BTBooster::isColumnExists($this->table, 'created_at')) {
            $has_created_at = true;
        }
        ...
        foreach ($rows as $value) {
            ...
            foreach ($select_column as $sk => $s) {
                ...
                if (BTBooster::isForeignKey($colname)) {
                    ...
                    if (intval($value->$s)) {
                        ...
                        $relation_table = BTBooster::getTableForeignKey($colname);
                        ...
                        if (BTBooster::isColumnExists($relation_table, 'created_at')) {
    ...
    public function postDoUploadImportData()
    {
        ...
        if (Request::hasFile('userfile')) {
            ...
            $filePath = 'uploads/'.BT::myId().'/'.date('Y-m');
            ...
            $url = BTBooster::mainpath('import-data').'?file='.base64_encode($url_filename);
    ...
    public function postActionSelected()
    {
        ...
        if (! $id_selected) {
            BTBooster::redirect($_SERVER['HTTP_REFERER'], cbLang("alert_select_a_data"), 'warning');
        }

        if ($button_name == 'delete') {
            if (! BTBooster::isDelete()) {
                BTBooster::insertLog(cbLang("log_try_delete_selected", ['module' => BTBooster::getCurrentModule()->name]));
                BTBooster::redirect(BTBooster::adminPath(), cbLang('denied_access'));
            }

            $this->hook_before_delete($id_selected);
            $tablePK = BT::pk($this->table);
            if (BTBooster::isColumnExists($this->table, 'deleted_at')) {
            ...
            BTBooster::insertLog(cbLang("log_delete", ['name' => implode(',', $id_selected), 'module' => BTBooster::getCurrentModule()->name]));
    ...
    public function getDeleteImage()
    {
        ...
        if (! BTBooster::isDelete() && $this->global_privilege == false) {
            BTBooster::insertLog(cbLang("log_try_delete_image", [
                'name' => $row->{$this->title_field},
                'module' => BTBooster::getCurrentModule()->name,
            ]));
            BTBooster::redirect(BTBooster::adminPath(), cbLang('denied_access'));
        ...
        BTBooster::insertLog(cbLang("log_delete_image", [
            'name' => $row->{$this->title_field},
            'module' => BTBooster::getCurrentModule()->name,
        ]));

        BTBooster::redirect(Request::server('HTTP_REFERER'), cbLang('alert_delete_data_success'), 'success');
    }
    ...
    public function postUploadSummernote()
    {
        $this->cbLoader();
        $name = 'userfile';
        if ($file = BTBooster::uploadFile($name, true)) {
            echo asset($file);
        }
    }
