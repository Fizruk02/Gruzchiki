...
class ApiCustomController extends BTController
{
    ...
    function getIndex()
    {
        ...
        if (! BTBooster::isSuperadmin()) {
            BTBooster::insertLog(cbLang("log_try_view", ['name' => 'API Index', 'module' => 'API']));
    ...
    function getDownloadPostman()
    {
        ...
        $data['variables'] = [];
        $data['info'] = [
            'name' => BTBooster::getSetting('appname').' - API',
        ...
        return \Response::make($json, 200, [
            ...
            'Content-Disposition' => 'attachment; filename='.BTBooster::getSetting('appname').' - API For POSTMAN.json',
        ]);
    }
    ...
    public function getEditApi($id)
    {
        ...
        if (! BTBooster::isSuperadmin()) {
            BTBooster::insertLog(cbLang("log_try_view", ['name' => 'API Edit', 'module' => 'API']));
            BTBooster::redirect(BTBooster::adminPath(), cbLang('denied_access'));
        }
    ...
    {
        BTBooster::valid(['id', 'status'], 'view');
    ...
    function getColumnTable($table, $type = 'list')
    {
        ...
        $cols = BTBooster::getTableColumns($table);
        ...
        foreach ($result as $ro) {
            ...
            $type_field = BTBooster::getFieldType($table, $ro);
                ...
                if (substr($ro, 0, 3) == 'id_') {
                    ...
                    foreach ($t2 as $t) {
                        if ($t != 'id' && $t != 'created_at' && $t != 'updated_at' && $t != 'deleted_at') {
                            ...
                            $type_field = BTBooster::getFieldType($table2, $t);
    ...
    function postSaveApiCustom()
    {
        ...
        if (Request::get('id')) {
            ...
            BTBooster::generateAPI($controllerName, $a['tabel'], $a['permalink'], $a['method_type']);

            DB::table('cms_apicustom')->insert($a);
        }

        return redirect(BTBooster::mainpath())->with(['message' => 'Yeay, your api has been saved successfully !', 'message_type' => 'success']);
    }
