...
use App\Constructor\fonts\Fontawesome;

class ModulsController extends BTController
{
    public function cbInit()
    {
        ...
        $row = BTBooster::first($this->table, BTBooster::getCurrentId());
        ...
        if (BTBooster::getCurrentMethod() == 'getAdd' || BTBooster::getCurrentMethod() == 'postAddSave') {
        ...
        $this->addaction[] = [
            'label' => 'Module Wizard',
            'icon' => 'fa fa-wrench',
            'url' => BTBooster::mainpath('step1').'/[id]',
            "showIf" => "[is_protected] == 0",
        ];

        $this->index_button[] = ['label' => 'Generate New Module', 'icon' => 'fa fa-plus', 'url' => BTBooster::mainpath('step1'), 'color' => 'success'];
    }
    ...
    {
        $columns = BTBooster::getTableColumns($table);
    ...
    public function getStep1($id = 0)
    {
        ...
        $row = BTBooster::first($this->table, ['id' => $id]);
    ...
    public function getStep2($id)
    {
        ...
        $columns = BTBooster::getTableColumns($row->table_name);
    ...
    public function postStep2()
    {
        ...
        if (! Request::get('id')) {
            ...
            $controller = BTBooster::generateController($table_name, $path);
            ...
            if ($controller && Request::get('create_menu')) {
                ...
                $id_cms_menus = DB::table('cms_menus')->insertGetId([
                    ...
                    'type' => 'Route',
                    'is_active' => 1,
                    'id_cms_privileges' => BTBooster::myPrivilegeId(),
                    'sorting' => $parent_menu_sort,
                    'parent_id' => 0,
                ]);
                DB::table('cms_menus_privileges')->insert(['id_cms_menus' => $id_cms_menus, 'id_cms_privileges' => BTBooster::myPrivilegeId()]);
            }

            $user_id_privileges = BTBooster::myPrivilegeId();
            ...
            $roles = DB::table('cms_privileges_roles')->where('id_cms_privileges', BTBooster::myPrivilegeId())->join('cms_moduls', 'cms_moduls.id', '=', 'id_cms_moduls')->select('cms_moduls.name', 'cms_moduls.path', 'is_visible', 'is_create', 'is_read', 'is_edit', 'is_delete')->get();
    ...
    public function getTypeInfo($type = 'text')
    {
        ...
        echo file_get_contents(base_path('app/Constructor/views/default/type_components/'.$type.'/info.json'));
    }
    ...
    public function postAddSave()
    {
        ...
        if ($this->arr['controller'] == '') {
            $this->arr['controller'] = BTBooster::generateController(Request::get('table_name'), $route_basename);
        }
...
//            $parent_menu_id = DB::table('cms_menus')->max('id') + 1;
        ...
        if (Request::get('return_url')) {
            BTBooster::redirect(Request::get('return_url'), cbLang("alert_add_data_success"), 'success');
        } else {
    ...
    public function postEditSave($id)
    {
        ...
        BTBooster::redirect(Request::server('HTTP_REFERER'), cbLang('alert_update_data_success'), 'success');
    }
}
