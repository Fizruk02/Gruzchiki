...
class NotificationsController extends BTController
{
    ...
    {
        $query->where('id_cms_users', BTBooster::myId());
    }

    public function getLatestJson()
    {

        $rows = DB::table('cms_notifications')->where('id_cms_users', 0)->orWhere('id_cms_users', BTBooster::myId())->orderby('id', 'desc')->where('is_read', 0)->take(25);
