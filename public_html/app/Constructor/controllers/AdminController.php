...
class AdminController extends BTController
{
    function getIndex()
    {
        $data = [];
    ...
    public function postUnlockScreen()
    {
        ...
        if (\Hash::check($password, $users->password)) {
            ...
            return redirect(BTBooster::adminPath());
        } else {
    ...
    public function postLogin()
    {
        ...
        if (\Hash::check($password, $users->password)) {
            ...
            BTBooster::insertLog(cbLang("log_login", ['email' => $users->email, 'ip' => Request::server('REMOTE_ADDR')]));

            $cb_hook_session = new \App\Http\Controllers\BTHook;
    ...
    public function postForgot()
    {
        ...
        $appname = BTBooster::getSetting('appname');
        $user = BTBooster::first(config('crudbooster.USER_TABLE'), ['email' => g('email')]);
        $user->password = $rand_string;
        BTBooster::sendEmail(['to' => $user->email, 'data' => $user, 'template' => 'forgot_password_backend']);

        BTBooster::insertLog(cbLang("log_forgot", ['email' => g('email'), 'ip' => Request::server('REMOTE_ADDR')]));
    ...
    public function getLogout()
    {
        ...
        $me = BTBooster::me();
        BTBooster::insertLog(cbLang("log_logout", ['email' => $me->email]));
