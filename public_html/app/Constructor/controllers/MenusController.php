...
class MenusController extends BTController
{
    public function cbInit()
    {
        ...
        if (@$row->type == 'Module') {
            $m = BTBooster::first('cms_moduls', ['path' => $row->path]);
            $id_module = $m->id;
        } elseif (@$row->type == 'Statistic') {
            $row->path = str_replace('statistic_builder/show/', '', $row->path);
            $m = BTBooster::first('cms_statistics', ['slug' => $row->path]);
            $id_statistic = $m->id;
        }

        if ($row) $this->script_js = "
			$(function() {
				var current_id = '$id';
    ...
    public function getIndex()
    {
        ...
        $id_cms_privileges = Request::get('id_cms_privileges');
        $id_cms_privileges = ($id_cms_privileges) ?: BTBooster::myPrivilegeId();
    ...
    public function hook_before_add(&$postdata)
    {
        ...
        if (! @$postdata['id_cms_privileges']) {
            $postdata['id_cms_privileges'] = BTBooster::myPrivilegeId();
        }
        $postdata['parent_id'] = 0;

        if ($postdata['type'] == 'Statistic') {
            $stat = BTBooster::first('cms_statistics', ['id' => $postdata['statistic_slug']]);
            $postdata['path'] = 'statistic_builder/show/'.$stat->slug;
        } elseif ($postdata['type'] == 'Module') {
            $stat = BTBooster::first('cms_moduls', ['id' => $postdata['module_slug']]);
        ...
        if ($postdata['is_dashboard'] == 1) {
            ...
            Cache::forget('sidebarDashboard'.BTBooster::myPrivilegeId());
        }
    }
