        ...
        */
        ...
        if ($parameters) {
            ...
            foreach ($parameters as $param) {
                ...
                if ($type == 'exists') {
                    ...
                    $table_exist = BTBooster::parseSqlTable($table_exist)['table'];
                ...
                if ($name == 'id') {
                    $table_exist = BTBooster::parseSqlTable($table)['table'];
                    $table_exist_pk = BTBooster::pk($table_exist);
        ...
        if ($action_type == 'list' || $action_type == 'detail' || $action_type == 'delete') {
            ...
            foreach ($responses as $resp) {
                ...
                if ($used == 0 && ! BTBooster::isForeignKey($name)) {
                    continue;
                }
                ...
                $name_tmp[] = $name;
                if (BTBooster::isForeignKey($name)) {
                    $jointable = BTBooster::getTableForeignKey($name);
                    $jointable_field = BTBooster::getTableColumns($jointable);
                    $jointablePK = BTBooster::pk($jointable);
            ...
            if (BTBooster::isColumnExists($table, 'deleted_at')) {
            ...
            $data->where(function ($w) use ($parameters, $posts, $table, $type_except) {
                foreach ($parameters as $param) {
                    ...
                    if ($required == '1') {
                        if (BTBooster::isColumnExists($table, $name)) {
            ...
            foreach ($input_validator as $k => $v) {
                if (BTBooster::isColumnExists($table, $k)) {
                    $row_assign[$k] = $v;
                }
            }
            ...
            if ($action_type == 'save_add') {
                if (BTBooster::isColumnExists($table, 'created_at')) {
                    $row_assign['created_at'] = date('Y-m-d H:i:s');
                }
            }

            if ($action_type == 'save_edit') {
                if (BTBooster::isColumnExists($table, 'updated_at')) {
            ...
            foreach ($parameters as $param) {
                ...
                if ($type == 'file' || $type == 'image') {
                    $row_assign[$name] = BTBooster::uploadFile($name, true);
                } elseif ($type == 'base64_file') {
                    $row_assign[$name] = BTBooster::uploadBase64($value);
            ...
            foreach ($parameters as $param) {
                ...
                $type = $param['type'];
                if ($type == 'ref') {
                    if (BTBooster::isColumnExists($config, 'id_'.$table)) {
                        DB::table($config)->where($name, $value)->update(['id_'.$table => $lastId]);
                    } elseif (BTBooster::isColumnExists($config, $table.'_id')) {
