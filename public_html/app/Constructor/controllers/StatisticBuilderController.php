...
class StatisticBuilderController extends BTController
{
    public function cbInit()
    {
        ...
        $this->addaction = [];
        $this->addaction[] = ['label' => 'Builder', 'url' => BTBooster::mainpath('builder').'/[id]', 'icon' => 'fa fa-wrench'];
    }

    public function getShowDashboard()
    {
        $this->cbLoader();
        $m = BTBooster::sidebarDashboard();
        $m->path = str_replace("statistic_builder/show/", "", $m->path);
        if ($m->type != 'Statistic') {
            redirect('/');
        }
        $row = BTBooster::first($this->table, ['slug' => $m->path]);
    ...
    public function getDashboard()
    {
        ...
        $menus= DB::table('cms_menus')
            ->whereRaw("cms_menus.id IN (select id_cms_menus from cms_menus_privileges where id_cms_privileges = '".BTBooster::myPrivilegeId()."')")
        ...
        $row = BTBooster::first($this->table, ['slug' => $slug]);
        $id_cms_statistics = @$row->id;
        $page_title = @$row->name;
    ...
    public function getBuilder($id_cms_statistics)
    {
        ...
        if (! BTBooster::isSuperadmin()) {
            BTBooster::insertLog(cbLang("log_try_view", ['name' => 'Builder', 'module' => 'Statistic']));
    ...
    public function postAddComponent()
    {
        ...
        BTBooster::insert('cms_statistic_components', $data);
    ...
    public function getEditComponent($componentID)
    {
        ...
        if (! BTBooster::isSuperadmin()) {
            BTBooster::insertLog(cbLang("log_try_view", ['name' => 'Edit Component', 'module' => 'Statistic']));
            BTBooster::redirect(BTBooster::adminPath(), cbLang('denied_access'));
        }

        $component_row = BTBooster::first('cms_statistic_components', ['componentID' => $componentID]);
    ...
    {
        if (! BTBooster::isSuperadmin()) {
            BTBooster::insertLog(cbLang("log_try_view", ['name' => 'Delete Component', 'module' => 'Statistic']));
            BTBooster::redirect(BTBooster::adminPath(), cbLang('denied_access'));
        }
