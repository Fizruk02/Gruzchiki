...
class PrivilegesController extends BTController
{
    public function cbInit()
    {
        ...
        $this->alert[] = [
            'message' => "You can use the helper <code>BTBooster::getMyPrivilegeId()</code> to get current user login privilege id, or <code>BTBooster::getMyPrivilegeName()</code> to get current user login privilege name",
            'type' => 'info',
        ];
    }
    ...
    public function getEdit($id, $path = null)
    {
        $this->cbLoader();

        $row = DB::table($this->table)->where("id", $id)->first();

        if (! BTBooster::isRead() && $this->global_privilege == false) {
            BTBooster::insertLog(cbLang("log_try_edit", [
                'name' => $row->{$this->title_field},
                'module' => BTBooster::getCurrentModule($path)->name,
            ]));
            BTBooster::redirect(BTBooster::adminPath(), cbLang('denied_access'));
        }
    ...
    public function postEditSave($id)
    {
        ...
        $row = BTBooster::first($this->table, $id);
        ...
        BTBooster::redirect(BTBooster::mainpath(), cbLang("alert_update_data_success", [
            'module' => "Privilege",
            'title' => $row->name,
        ]), 'success');
    }
    ...
    public function getDelete($id)
    {
        ...
        BTBooster::redirect(BTBooster::mainpath(), cbLang("alert_delete_data_success"), 'success');
    }
}
