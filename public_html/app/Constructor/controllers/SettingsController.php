...
class SettingsController extends BTController
{
    ...
    function getShow()
    {
        ...
        if (! BTBooster::isSuperadmin()) {
            BTBooster::insertLog(cbLang("log_try_view", ['name' => 'Setting', 'module' => 'Setting']));
    ...
    {
        $this->return_url = BTBooster::mainpath("show")."?group=".$posdata['group_setting'];
    }

    function getDeleteFileSetting()
    {
        $id = g('id');
        $row = BTBooster::first('cms_settings', $id);
        ...
        foreach ($setting as $set) {
            ...
            if (Request::hasFile($name)) {
                ...
                if ($set->content_input_type == 'upload_image') {
                    BTBooster::valid([$name => 'image|max:10000'], 'view');
                } else {
                    BTBooster::valid([$name => 'mimes:doc,docx,xls,xlsx,ppt,pptx,pdf,zip,rar|max:20000'], 'view');
                }
    ...
    function hook_before_add(&$arr)
    {
        ...
        $this->return_url = BTBooster::mainpath("show")."?group=".$arr['group_setting'];
    }
